...
import { ISuperfluid, ISuperfluidGovernance, ISuperAgreement, ISuperApp, SuperAppDefinitions, ContextDefinitions, BatchOperation, SuperfluidGovernanceConfigs, ISuperfluidToken, ISuperToken, ISuperTokenFactory } from "../interfaces/superfluid/ISuperfluid.sol";
...
contract Superfluid is UUPSProxiable, ISuperfluid, BaseRelayRecipient {
    ...
    function mapAgreementClasses(
        uint256 bitmap
    ) external view override returns (ISuperAgreement[] memory agreementClasses) {
        uint i;
        uint n;
    ...
    function updateSuperTokenFactory(ISuperTokenFactory newFactory) external override onlyGovernance {
    ...
    function updateSuperTokenLogic(ISuperToken token, address newLogicOverride) external override onlyGovernance {
    ...
    function getAppManifest(
        ISuperApp app
    ) external view override returns (bool isSuperApp, bool isJailed, uint256 noopMask) {
    ...
    function _callAppAction(
        ...
        uint256 value,
        bytes memory callData
    ) internal cleanCtx isAppActive(app) isValidAppAction(callData) returns (bytes memory returnedData) {
        // Build context data
        bytes memory ctx = _updateContext(
            Context({
                appCallbackLevel: 0,
    ...
    function callAppActionWithContext(
        ...
        bytes calldata callData,
        bytes calldata ctx
    ) external override requireValidCtx(ctx) isAppActive(app) isValidAppAction(callData) returns (bytes memory newCtx) {
        Context memory context = decodeCtx(ctx);
    ...
    function _batchCall(address msgSender, Operation[] calldata operations) internal {
        ...
        for (uint256 i = 0; i < operations.length; ++i) {
            ...
            if (operationType == BatchOperation.OPERATION_TYPE_ERC20_APPROVE) {
                ...
                (address recipient, uint256 amount, bytes memory userData) = abi.decode(
                    operations[i].data,
    ...
    function batchCall(Operation[] calldata operations) external payable override {
    ...
    function _decodeCtx(bytes memory ctx) private pure returns (Context memory context) {
        ...
        {
            uint256 callInfo;
            (callInfo, context.timestamp, context.msgSender, context.agreementSelector, context.userData) = abi.decode(
                ctx1,
                (uint256, uint256, address, bytes4, bytes)
            );
            (context.appCallbackLevel, context.callType) = ContextDefinitions.decodeCallInfo(callInfo);
        }
        {
            uint256 creditIO;
            (creditIO, context.appCreditUsed, context.appAddress, context.appCreditToken) = abi.decode(
                ctx2,
                (uint256, int256, address, ISuperfluidToken)
            );
