...
// Internal Libraries
import { Metadata } from "../libraries/allo/Metadata.sol";
...
contract SQFSuperFluidStrategy is BaseStrategy, ReentrancyGuard {
    ...
    function initialize(uint256 _poolId, bytes memory _data) external override {
        ...
        if (
            params.superfluidHost == address(0) ||
            params.allocationSuperToken == address(0) ||
            params.superfluidHost == address(0) ||
            params.passportDecoder == address(0)
        ) revert ZERO_ADDRESS();
    ...
    ) external virtual onlyPoolManager(msg.sender) onlyBeforeAllocationEnds {
        ...
        if (_previousFlowRate == 0) {
            ...
            if (scaledFlowRate > 0) {
                recipientTotalUnits =
                    ...
                    scaledFlowRate -
                    2 *
                    uint256(recipientTotalUnits * scaledFlowRate).sqrt();
            }
        } else {
            // updated a flow
